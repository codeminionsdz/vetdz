...
function TimeSlots() {
  ...
  for (let hour = START_HOUR; hour <= END_HOUR; hour++) {
    const d = new Date(1970, 0, 1, hour, 0, 0, 0);
    const label = d.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit", hour12: false });
    slots.push(
      <div
        key={hour}
        className="relative"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 font-medium text-foreground truncate">
        ...
        <span className="truncate">{appointment.patientName || "Patient inconnu"}</span>
      </div>
      {height >= 36 && (
        <div className="text-muted-foreground truncate mt-0.5">
          {appointment.typeName || "Rendez-vous"} &middot;{" "}
...
function AppointmentDetailPopover({
  ...
  if (current === "scheduled" || current === "confirmed") {
    statusActions.push({ label: "Présent", status: "checked_in", variant: "default" });
    statusActions.push({ label: "Absence", status: "no_show", variant: "outline" });
    statusActions.push({ label: "Annuler", status: "cancelled", variant: "destructive" });
  } else if (current === "checked_in" || current === "in_exam") {
    statusActions.push({ label: "Sortie", status: "checked_out", variant: "default" });
    if (current === "checked_in") {
      statusActions.push({ label: "En examen", status: "in_exam", variant: "outline" });
    }
    statusActions.push({ label: "Absence", status: "no_show", variant: "outline" });
  } else if (current === "no_show" || current === "cancelled") {
    statusActions.push({ label: "Replanifier", status: "scheduled", variant: "outline" });
  }
...
function generateTimeSlots(): { label: string; value: string }[] {
  ...
  for (let hour = 8; hour <= 17; hour++) {
    for (const min of [0, 30]) {
      ...
      const dt = new Date(1970, 0, 1, hour, min, 0);
      const label = dt.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit", hour12: false });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h3 className="text-sm font-semibold">Nouveau rendez-vous</h3>
          <button
            type="button"
            onClick={onClose}
            className="rounded-md p-1 hover:bg-muted transition-colors"
        ...
        <div className="px-4 py-3 space-y-4">
                      ...
                      >
                        ...
                        <div className="text-xs text-muted-foreground">
                          ...
                          {(p.clientFirstName || p.clientLastName) && (
                            <> &middot; Propriétaire&nbsp;: {[p.clientFirstName, p.clientLastName].filter(Boolean).join(" ")}</>
                          )}
                        </div>
                      </button>
                    ))}
          ...
          {/* Appointment Type */}
          <div>
            <label className="text-xs font-medium text-muted-foreground">Type de rendez-vous</label>
            <select
              value={typeId}
          ...
          {/* Doctor */}
          <div>
            <label className="text-xs font-medium text-muted-foreground">Docteur</label>
            <select
              value={doctorId}
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-xl font-semibold">Agenda</h2>
          <p className="text-sm text-muted-foreground">Calendrier des rendez-vous</p>
        </div>
      </div>
      ...
      {view === "week" ? (
        <div className="mt-4 rounded-lg border border-dashed border-border bg-card p-12 text-center">
          ...
          <p className="mt-2 text-muted-foreground">Vue semaine bientôt disponible</p>
        </div>
      ) : isLoading ? (
