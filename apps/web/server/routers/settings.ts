...
export const settingsRouter = createRouter({
  ...
  getPractice: adminProcedure.query(async ({ ctx }) => {
    ...
    const [location] = await ctx.db
      .select({ wilaya: locations.wilaya, commune: locations.commune })
      .from(locations)
      .where(and(eq(locations.practiceId, ctx.practiceId), locations.isPrimary))
      .limit(1);
  ...
  updatePractice: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { wilaya, commune, ...practiceData } = input;
      ...
      if (Object.keys(practiceData).length > 0) {
        const [updated] = await ctx.db
          ...
          .where(eq(practices.id, ctx.practiceId))
          .returning();
        updatedPractice = updated!;
      }

      // Update primary location wilaya/commune if provided
      ...
      return updatedPractice ?? (await ctx.db.select().from(practices).where(eq(practices.id, ctx.practiceId)).limit(1))[0]!;
    }),
