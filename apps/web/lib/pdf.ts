...
export function generateInvoicePdf(data: InvoiceData): jsPDF {
  ...
  if (data.balanceDue) {
    ...
    const totalVal = parseFloat(cleanNum(data.total)) || 0;
    const paidVal = parseFloat(cleanNum(data.paidAmount)) || 0;
    balanceStr = `${(totalVal - paidVal).toLocaleString("fr-DZ")} DA`;
  }
  doc.setFont(FONT, "bold");
  setColor(doc, COLOR_TEAL);
  doc.text("Balance Due:", totalsX, y);
  doc.text(balanceStr, totalsValX, y, { align: "right" });
